...
| `/risk` | 直接触发 Risk Analyst Agent，并结合投资画像判断是否偏离纪律 |
| `/news <关键词>` | 直接触发 News Agent，例如 `/news AAPL earnings` |
| `/brief` | 立即生成一次开盘前简报，包含核心指标、今天只看三件事、相比上次快照变化、主要持仓和风险提醒 |
| `/profile` | 查看投资画像 |
| `/profile setup` | 查看投资画像设置模板和字段解释 |
...
- 加入“相比上次快照”变化，展示净值、现金和主要持仓动作，让日常简报回答“今天和上次有什么不同”。
...
- 结合 `/profile` 判断组合是否符合用户纪律，例如单一持仓上限、现金底线和风险偏好。
